...
- A new RenderContext for WPF (DrawingRenderContext) that renders into DrawingGroup instead of creating WPF elements. Adding WPF elements during render can cause flickering and block rendering when there are a lot of series. Changed WPF PlotView to use this render context.
